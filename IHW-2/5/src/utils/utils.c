...
int close_semaphore(__attribute__ ((unused)) sem_t* sem) { return (close_memory(sem) == -1) ? -1 : 0; }
int delete_semaphore(const char* name, sem_t* sem)
{
    int status = 0;
    if (sem && sem_destroy(sem) == -1) status = -1;
...
int clear_state(struct State state)
{
    ...
    if (delete_semaphore(door_in_semaphore, state.door_in) == -1) status = -1;
    if (delete_semaphore(reception_in_semaphore, state.reception_in) == -1) status = -1;
    if (delete_semaphore(reception_out_semaphore, state.reception_out) == -1) status = -1;
    if (delete_semaphore(door_out_semaphore, state.door_out) == -1) status = -1;
    // Delete the memory
