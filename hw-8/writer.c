...
void sigfunc(int sig) {
  ...
  if(sig == SIGINT) {
    if (buffer->first_reader) kill(buffer->first_reader, SIGTERM);
    if (buffer->second_reader) kill(buffer->second_reader, SIGTERM);
...
int main() {
 ...
 if ( (buf_id = shm_open(shar_object, O_CREAT|O_RDWR, 0666)) == -1 ) {
  ...
  buffer->readers = 0;
  buffer->first_reader = 0;
  buffer->second_reader = 0;
  ...
  while (1) {
    ...
    if(sem_wait(writeMutex) == -1) {
      perror("sem_wait: Incorrect wait of mutex");
      exit(-1);
    };

    // Поиск следующей свободной ячейки для записи. Она должна быть. Иначе сюда не попасть
    do {
      ++buffer->writerI;
      if (buffer->writerI == BUF_SIZE) buffer->writerI = 0;
      printf("    --> checking store: store[%d] = %d\n", buffer->writerI, buffer->store[buffer->writerI]);
    } while(buffer->store[buffer->writerI] != -1);
    // Запись в первую свободную ячейку
    int value = rand() % 11;
    buffer->store[buffer->writerI] = value; // число от 0 до 10

    const int writerISave = buffer->writerI;
    ...
    // Выход из критической секции записи
    if(sem_post(writeMutex) == -1) {
      perror("sem_post: Incorrect post of write mutex semaphore");
      exit(-1);
    };
    
    // Вывод информации об операции записи
    pid_t pid = getpid();
    printf("Producer %d writes value = %d to cell [%d]\n", pid, value, writerISave);
    sleep(1);
  }
  return 0;
}
