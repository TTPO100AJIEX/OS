...
static int handleCome(struct ComeRequest request)
{
    ...
    else
    {
        ...
        log("Registered the visitor %d into the room %d\n", request.id, room);
        // Construct the response message
        struct Message message = {
            .type = COME_RESPONSE,
...
{
    log("Visitor with id %d wants to leave\n", request.id);
...
int main(void)
{
    ...
    log("Initialized %d rooms\n", 25);

    while (running)
    {
        // Wait for the next request and get it
        struct Message message;
        if (get_request(state, &message) == -1) { printf("Failed to get the request\n"); raise(SIGINT); }
        // Check the type of the request and do the required handling
        switch (message.type)
        {
            case COME_REQUEST:
            {
                if (handleCome(message.data.come_request) == -1) { printf("Failed to process a come request\n"); raise(SIGINT); }
                break;
            }
            case LEAVE_REQUEST:
            {
                if (handleLeave(message.data.leave_request) == -1) { printf("Failed to process a leave request\n"); raise(SIGINT); }
                break;
            }
            default: { }
        }
