...
int main(int argc, char** argv) // <IP> <Port> <Gender (m/f)> <Time>
{
    ...
    log("Sent client_type to the server\n");

    // Sleep for more realistic interaction
    nanosleep(&to_sleep, NULL);

    // Send the request to get a room
    if (send(client, &gender, sizeof(gender), 0) != sizeof(gender)) { perror("Failed to send the request with the gender"); close(client); return 1; }
