...
int main(int argc, char** argv)
{
    ...
    if (pipe(writerPipe) == -1) { perror("Failed to create a writer pipe"); return -1; }
    
    pid_t reader = fork();
    if (reader == -1) { perror("Failed to create the reader process"); return -1; }
    if (reader == 0)
    {
        if (close(writerPipe[0]) == -1) { perror("Reader - failed to close the reading side of writer pipe"); return -1; }
        if (close(writerPipe[1]) == -1) { perror("Reader - failed to close the writing side of writer pipe"); return -1; }
        if (close(readerPipe[0]) == -1) { perror("Reader - failed to close the reading side of reader pipe"); return -1; }

        int inputFile = open(argv[1], O_RDONLY);
        if (inputFile == -1) { perror("Failed to open an input file"); close(readerPipe[1]); return -1; }

        input(inputFile, readerPipe[1]);

        if (close(readerPipe[1]) == -1) { perror("Reader - failed to close the writing side of reading pipe"); return -1; }
        return 0;
    }

    
    pid_t solver = fork();
    if (solver == -1) { perror("Failed to create the solver process"); kill(reader, SIGINT); return -1; }
    if (solver == 0)
    {
        if (close(readerPipe[1]) == -1) { perror("Solver - failed to close the writing side of reader pipe"); return -1; }
        if (close(writerPipe[0]) == -1) { perror("Solver - failed to close the reading side of writer pipe"); return -1; }
        
        solve(readerPipe[0], writerPipe[1]);

        if (close(readerPipe[0]) == -1) { perror("Solver - failed to close the reading side of reader pipe"); return -1; }
        if (close(writerPipe[1]) == -1) { perror("Solver - failed to close the writing side of writer pipe"); return -1; }
        return 0;
    }
    
    pid_t writer = fork();
    if (writer == -1) { perror("Failed to create the writer process"); kill(reader, SIGINT); kill(solver, SIGINT); return -1; }
    if (writer == 0)
    {
        if (close(readerPipe[0]) == -1) { perror("Writer - failed to close the reading side of reader pipe"); return -1; }
        if (close(readerPipe[1]) == -1) { perror("Writer - failed to close the writing side of reader pipe"); return -1; }
        if (close(writerPipe[1]) == -1) { perror("Writer - failed to close the writing side of writer pipe"); return -1; }

        int outputFile = open(argv[2], O_CREAT | O_WRONLY, 0666);
        if (outputFile == -1) { perror("Failed to open an output file"); close(writerPipe[0]); return -1; }
        if (ftruncate(outputFile, 0) == -1) { perror("Failed to clear the output file"); close(writerPipe[0]); return -1; }

        output(writerPipe[0], outputFile);

        if (close(writerPipe[0]) == -1) { perror("Writer - failed to close the reading side of writer pipe"); return -1; }
        return 0;
    }
