...
void sigfunc(int sig) {
  ...
  if(sig == SIGINT) {
    ...
    if (buffer->first_reader != getpid()) kill(buffer->first_reader, SIGTERM);
    if (buffer->second_reader != getpid()) kill(buffer->second_reader, SIGTERM);
...
int main() {
  ...
  buffer->readers++;  // подъем флага, запрещающего доступ другим
  if (buffer->readers == 1)
  {
    buffer->first_reader = getpid();
    printf("Reader %d: I am first for this work! :)\n", getpid());
  }
  if (buffer->readers == 2)
  {
    buffer->second_reader = getpid();
    printf("Reader %d: I am second for this work! :)\n", getpid());
  }
  ...
  while (1) {
    ...
    if(sem_wait(readMutex) == -1) {
      perror("sem_wait: Incorrect wait of busy semaphore");
      exit(-1);
    };

    // Поиск следующей занятой ячейки для записи. Она должна быть. Иначе сюда не попасть
    do {
      ++buffer->readerI;
      if (buffer->readerI == BUF_SIZE) buffer->readerI = 0;
      printf("    --> checking store: store[%d] = %d\n", buffer->readerI, buffer->store[buffer->readerI]);
    } while(buffer->store[buffer->readerI] == -1);

    // Получение значения из читаемой ячейки
    int result = buffer->store[buffer->readerI];
    buffer->store[buffer->readerI] = -1;  // данные прочитаны

    const int readerISave = buffer->readerI;
    ...
    // Выход из критической секции чтения
    if(sem_post(readMutex) == -1) {
      perror("sem_post: Incorrect post of read mutex semaphore");
      exit(-1);
    };
