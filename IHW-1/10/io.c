...
int main(int argc, char** argv)
{
    ...
    if (writer == 0)
    {
        ...
        if (writerQueue == -1) { perror("Writer - failed to create a writer message queue"); return -1; }

        int outputFile = open(argv[2], O_CREAT | O_WRONLY, 0666);
        if (outputFile == -1) { perror("Writer - failed to open an output file"); msgctl(writerQueue, IPC_RMID, NULL); return -1; }
        if (ftruncate(outputFile, 0) == -1) { perror("Writer - failed to clear the output file"); close(outputFile); msgctl(writerQueue, IPC_RMID, NULL); return -1; }
